...
<p align="center">
  <em>Capacités de web crawling et RAG pour les agents IA et assistants de codage IA</em>
</p>
...
1. **Cloner le dépôt** :
   ```bash
   git clone https://github.com/votre-utilisateur/sti_map_generator.git
   cd sti_map_generator/docker/services/mcp-crawl4ai-rag
   ```

2. **Configurer les variables d'environnement** :
   ```bash
   cp .env.example .env.local
   # Éditer le fichier .env.local avec vos clés d'API
   nano .env.local
   ```
...
4. **Vérifier le statut** :
   Le script affichera l'URL du service et les endpoints disponibles une fois le démarrage terminé.

5. **Accéder aux endpoints** :
...
- Clé d'API [OpenAI](https://platform.openai.com/api-keys) (optionnel)

Une implémentation puissante du [Model Context Protocol (MCP)](https://modelcontextprotocol.io) intégrée avec [Crawl4AI](https://crawl4ai.com) et [Supabase](https://supabase.com/) pour fournir aux agents IA et assistants de codage IA des capacités de web crawling et RAG avancées.

Avec ce serveur MCP, vous pouvez <b>extraire n'importe quelle information</b> et ensuite <b>utiliser ces connaissances n'importe où</b> pour RAG.
...
Ce serveur MCP fournit des outils permettant aux agents IA de crawler des sites web, de stocker le contenu dans une base de données vectorielle (Supabase) et d'effectuer des requêtes RAG sur le contenu collecté. Il suit les bonnes pratiques pour la construction de serveurs MCP basées sur le [modèle de serveur MCP Mem0](https://github.com/coleam00/mcp-mem0/).
